...
type AOA = any[][];
@Component({
  selector: 'app-create-pothole-work',
  templateUrl: './create-pothole-work.component.html',
  styleUrls: ['./create-pothole-work.component.scss'],
})
export class CreatePotholeWorkComponent {
  form: FormGroup;
  locId: string;
  wardList: wardModel[];
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap.subscribe((param) => {
      ...
      if (id) {
        this.pageTitle = 'Edit Pothole Work';
        this.potholeWorkService.getLocationById(id).subscribe((result) => {
          debugger;
            if (result != null) {
              if (result.status === 200) {
                console.log(result.data[0]);
  ...
  onSubmit() {
    ...
    if (this.locId) {
      ...
      let objMasticWork = {
        ...
        locationName: this.form.value.userSelectionRadio == 'two'? this.form.value.locationName: 
                      this.masticRoadsList.filter((f) => f._id == this.form.value.location)[0].locationName,
      ...
      debugger;
      this.potholeWorkService.updatePotholeWork(this.locId, objMasticWork).subscribe(
          (result) => {
            if (result != null) {
    ...
    } else {
      {
        ...
        debugger;
        this.masticworkService.addMasticWork(objMasticWork)
          .subscribe((result) => {
            if (result) {
              if (result.status == 201) {
                Swal.fire({
      ...
      if (this.data[i].length) {
        if (this.data[i][1] != '') {
          ...
          if (this.data[i][12] === '') {
            ...
            this.dataDate = this.convertExcelDateToJSDate(this.data[i][12]);
            debugger;
          }
          debugger;
          if (this.data[i][15] === '') {
            ...
            this.modifiedOn = this.convertExcelDateToJSDate(this.data[i][15]);
          }
          debugger;
          let attr1 = {
  ...
  convertExcelDateToJSDate(excelDate: number): string | null {
    if (!excelDate) return null;
  
    const days = Math.floor(excelDate - 25569);
    const seconds = Math.round((excelDate - 25569) * 86400);
    const date = new Date(days * 86400 * 1000 + (seconds % 86400) * 1000);
  ...
  addPotholeWork(objLocation) {
    ...
    debugger;
    this.potholeWorkService.addPotholeWork(this.objLocation).subscribe(
      (result) => {
        debugger;
        if (result != null) {
          if (result.status === 201) {
