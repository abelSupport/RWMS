...
import { PotholeUser } from 'src/app/core/models/IPotholeUser';
...
@Component({
  selector: 'app-create-pothole-user',
  templateUrl: './create-pothole-user.component.html',
  styleUrls: ['./create-pothole-user.component.scss']
})
export class CreatePotholeUserComponent implements OnInit {
  pageTitle: string = 'Create User';
  customStylesValidated = false;
  userModel: PotholeUser;
  form: FormGroup;
  idforEdit: string;
  roleList: roleModel[];
  userRole: String;
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap.subscribe((params) => {
      ...
      if (id) {
        ...
        debugger;
        this.potholeUserService.getUserById(id).subscribe(
          (result) => {
            debugger;
            if (result != null) {
              if (result.status === 200) {
                ...
                this.form.patchValue({
                  ...
                  ecNo:this.userModel.ecNo,
                  electrolWardNo: this.userModel.electrolWardNo,
                  personalMobileNo:this.userModel.personalMobileNo,
                  simAlloted:this.userModel.simAlloted,
                  imsiAlloted:this.userModel.imsiAlloted,
    ...
    if (this.form.valid) {
      if (this.idforEdit) {
        ...
        this.userModel = {
          ...
          ecNo: this.form.value.ecNo,
          electrolWardNo: this.form.value.electrolWardNo,
          personalMobileNo: this.form.value.personalMobileNo,
          simAlloted: this.form.value.simAlloted,
          imsiAlloted: this.form.value.imsiAlloted,
        ...
        debugger;
        this.potholeUserService.updatePotholeUser( this.userModel).subscribe(
          (result) => {
            if (result != null) {
              if (result.status === 201) {
                ...
                this.router.navigate(['user/potholelist']);
              }
            } else {
              Swal.fire({
                title: 'Seesion Expired',
      ...
      if (this.data[i].length) {
        ...
        if (this.data[i][0] != '') {
          ...
          let attr1 = {
            ...
            ecNo:this.data[i][10],
            electrolWardNo:this.data[i][11],
            personalMobileNo:this.data[i][12],
            simAlloted:this.data[i][13],
            imsiAlloted:this.data[i][14],
  ...
  addUser(userModel1) {
    debugger;
    this.potholeUserService.addPotholeUser(userModel1).subscribe(
      (result) => {
        if (result != null) {
          if (result.status === 201) {
            this.router.navigate(['user/potholelist']);
